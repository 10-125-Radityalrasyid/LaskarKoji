...
def is_on_path_close(diamond_position, bot_position, base_position, threshold):
    on_path = (diamond_position.x == bot_position.x == base_position.x) or (
        diamond_position.y == bot_position.y == base_position.y)
    close_to_path = calc_distance(diamond_position, bot_position) <= threshold or calc_distance(
        diamond_position, base_position) <= threshold
    return on_path or close_to_path


class Samurai(BaseLogic):
    def __init__(self):
    ...
    def next_move(self, board_bot: GameObject, board: Board):
        ...
        if props.diamonds >= props.inventory_size - 1:
            ...
            if distance_to_base_via_teleport < distance_to_base:
                ...
                path_diamonds_with_1_points = [diamond for diamond in diamonds_with_1_point if is_on_path_close(
